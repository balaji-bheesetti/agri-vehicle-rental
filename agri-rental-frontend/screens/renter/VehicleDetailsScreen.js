...
import {
  ...
  FlatList,
  RefreshControl,
  Image, // FIX 1: Image component is now imported
} from 'react-native';
...
export default function RenterHomeScreen({ navigation }) {
  ...
  // FIX 2: Correctly implement useFocusEffect for data fetching
  useFocusEffect(
    useCallback(() => {
      const fetchData = async () => {
        // Don't set loading to true on pull-to-refresh
        if (!refreshing) {
          setIsLoading(true);
        }
      ...
      return () => {};
    }, [refreshing]) // Dependency on refreshing to allow pull-to-refresh
  );
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.categoriesGrid}>
            {/* FIX 3: Inline rendering is safer and clearer */}
